### Hashcat
Hashcat is an open-source, advanced password recovery tool used to crack password hashes. Its primary purpose is to use the power of GPUs (Graphics Processing Units) or CPUs to perform brute-force, dictionary, and hybrid attacks against password hashes. Hashcat is commonly employed in penetration testing, forensic analysis, and security auditing to assess the strength of passwords and to recover lost or forgotten passwords.

The tool supports a wide range of hashing algorithms and formats, making it versatile and applicable to various scenarios. Some of the supported hash types include MD5, SHA1, SHA256, SHA512, NTLM, bcrypt, scrypt, and many others.

#### Usage
1. **Installation:** Hashcat is available for different platforms, including Windows, Linux, and macOS. You can download it from the official Hashcat website or install it using package managers like APT or YUM on Linux.    
2. **Obtain Hashes:** Before using Hashcat, you need to obtain the password hashes you want to crack. These hashes are typically stored in databases or configuration files as a security measure instead of storing plaintext passwords.
3. **Prepare Wordlists or Rules:** Hashcat can use wordlists (dictionaries) containing potential passwords to perform dictionary attacks. You can create custom wordlists or use existing ones. Additionally, Hashcat allows the use of "rules" to modify and combine words from the wordlist, increasing the chances of cracking complex passwords.<br>[SecLists Repo](https://github.com/danielmiessler/SecLists)
4. **Select Attack Mode:** Hashcat supports various attack modes, such as straight brute-force, dictionary attacks, hybrid attacks, mask attacks (when you have an idea of the password pattern), and others. You need to choose the appropriate attack mode based on your scenario.
5. **Configure Hashcat Options:** Set up the attack parameters, including the hash type, the path to the hashes file, the wordlist or mask to use, and any additional rules or attack-specific options.
6. **Start the Attack:** Launch the Hashcat attack by running the appropriate command in the terminal or command prompt. For example:
	```bash
	hashcat -m 1000 -a 0 hashes.txt wordlist.txt
	```
    - In this example, `-m` specifies the hash type (NTLM in this case), `-a` indicates the attack mode (0 for straight brute-force), `hashes.txt` is the file containing the hashes, and `wordlist.txt` is the wordlist to use.
7. **Monitor Progress:** Hashcat will start attempting to crack the password hashes using the selected attack mode. It will display the progress and any recovered passwords in real-time.
8. **Retrieve Cracked Passwords:** Once Hashcat successfully cracks a password, it will display the plaintext password alongside the corresponding hash.
...
```bash
# 5600 | NetNTLMv2
$ hashcat -m 5600 hash.txt -w rockyou.txt
```